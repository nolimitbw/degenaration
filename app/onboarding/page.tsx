    ...
    <AppShell>
      <div className="mx-auto max-w-2xl">
        <div className="mb-8 flex items-center gap-2" aria-label={`Onboarding step ${step + 1} of ${STEPS.length}`}>
          {STEPS.map((label, index) => (
            <div key={label} className="flex flex-1 items-center gap-2">
              <div className={`grid h-7 w-7 shrink-0 place-items-center rounded-full font-mono t-label ${index < step ? "bg-gold-400 text-[#17110c]" : index === step ? "border-2 border-gold-400 text-gold-400" : "border border-edge text-dim"}`}>{index < step ? <Check size={14} /> : index + 1}</div>
        ...
        <div className="rounded-lg border border-edge bg-panel p-6">
          ...
          <h1 className="mt-1 t-title font-bold">{STEPS[step]}</h1>

          {step === 0 && (
            <div className="mt-4 space-y-4 t-body text-dim">
              ...
              {!authenticated && <button onClick={login} disabled={!ready} className="inline-flex min-h-11 sm:min-h-10 items-center gap-2 rounded-md border border-edge px-4 t-label font-bold transition hover:border-gold-400 disabled:opacity-50"><LogIn size={15} /> Sign in to continue</button>}
          ...
          {step === 1 && (
            <div className="mt-4">
              {authenticated && wallet ? <div className="rounded-md border border-gold-400/40 bg-void p-4"><div className="flex items-center gap-2 font-bold"><WalletCards size={17} className="text-gold-400" /> Solana wallet ready</div><code className="mt-2 block truncate font-mono t-label text-dim">{wallet}</code><p className="mt-2 t-label text-dim">Wallet keys remain secured by your wallet provider. Delegated access is optional and managed separately.</p></div> : <div className="rounded-md border border-edge bg-void p-4"><p className="t-body text-dim">Open Privy sign-in to create or link a supported Solana wallet.</p><button onClick={login} disabled={!ready} className="mt-3 inline-flex min-h-11 sm:min-h-10 items-center gap-2 rounded-md bg-gold-400 px-4 t-label font-bold text-[#17110c] disabled:opacity-50"><LogIn size={15} /> Open sign in</button></div>}
            </div>
          )}
          ...
          {step === 2 && (
            <div className="mt-4">
              ...
              <p className="mt-2 t-body text-dim">Fund your wallet and pick a source when you&apos;re ready. Nothing runs on its own until you activate a bot.</p>
              <div className="mt-5 flex flex-wrap gap-2">
                <Link href="/bots" className="inline-flex min-h-11 sm:min-h-10 items-center gap-2 rounded-md bg-gold-400 px-4 t-label font-bold text-[#17110c]">Browse bots <ArrowRight size={14} /></Link>
                <Link href="/wallet" className="inline-flex min-h-11 sm:min-h-10 items-center rounded-md border border-edge px-4 t-label font-bold transition hover:border-gold-400">Fund wallet</Link>
              </div>
            </div>
          )}
