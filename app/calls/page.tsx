...
export default function Calls() {
  ...
  async function persist(id: string, on: boolean) {
    ...
    await saveSubscription({
      ...
      daily_cap_sol: c.dailyCap, enabled: on
    });
    setSaved(id); toast("Settings saved for this group"); setTimeout(() => setSaved(null), 1500);
  }
    ...
    <AppShell>
      ...
      {loaded && groups.length === 0 && (
        <div className="mt-6 grid place-items-center rounded-lg border border-edge bg-panel/40 py-12 text-center">
          ...
          <p className="mt-1 max-w-md font-mono text-[11px] text-dim/70">Groups appear here after their owners apply and you approve them in Admin. Nothing is seeded — this is your real, empty list until you launch.</p>
          ...
          return (
            <div key={g.id} className={`gradient-border rounded-lg border p-5 transition ${on ? "border-toxic/60 shadow-toxic" : "border-edge"}`}>
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => setCopying(on ? copying.filter((x) => x !== g.id) : [...copying, g.id])}
