...
export default function BotManagerPage() {
  ...
  return (
    <AppShell>
      <PageHeader
        title="My bots"
        description="Pause, edit, and see how each one is doing."
        actions={
          <Link href={kind === "discord" ? "/bots/discord/new" : "/bots/kol/new"} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-gold-400 px-4 t-body font-medium text-[#17110c] transition hover:bg-gold-300 sm:min-h-10">
      ...
      {/* Stated above the list of bots, because this is the screen where a user checks whether
          their money is working. An "active" badge on a row is otherwise the strongest possible
          claim that it is. */}
      ...
      <section className="mt-5 flex flex-wrap items-center gap-3 border-y border-edge bg-panel/45 px-4 py-3">
        ...
        <div className="flex flex-wrap gap-4 t-label text-dim">
      ...
      <div className="mt-5">
        ...
        {bots == null && loadError && (
          <div className="rounded-md border border-edge bg-panel p-6">
            <p className="t-body font-semibold text-ink">Bots could not be loaded</p>
        ...
        {bots?.length === 0 && (
          <EmptyState
            ...
            action={<Link href={kind === "discord" ? "/bots/discord/new" : "/bots/kol/new"} className="inline-flex min-h-11 sm:min-h-10 items-center rounded-md bg-gold-400 px-4 t-body font-semibold text-[#17110c]">Create bot</Link>}
          />
        )}
        {!!bots?.length && (
            ...
            <table className="w-full min-w-[1280px] text-left">
              ...
              <tbody>
                {bots.map((bot) => {
                  ...
                  return (
                    <tr key={bot.id} className="border-t border-edge bg-void/45 t-label transition hover:bg-panel">
                      ...
                      <td className="px-4 py-4">
                        ...
                        <p className="mt-1 max-w-48 truncate t-label text-dim">{bot.id}</p>
                      </td>
                      <td className="px-4 py-4">
                      ...
                      <td className="px-4 py-4 font-mono text-dim">
                        ...
                        <p className="mt-1 t-label">Gas {bot.networkFeesLamports == null ? "—" : formatSol(bot.networkFeesLamports)}</p>
                      </td>
                      ...
                      <td className="px-4 py-4 t-label text-dim">{new Date(bot.updated_at || bot.updatedAt || Date.now()).toLocaleDateString()}</td>
                      <td className="px-4 py-4">
        ...
        <div className="fixed inset-0 z-[80] grid place-items-center bg-black/75 p-4 backdrop-blur-sm" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && busy !== archiveTarget.id) setArchiveTarget(null); }}>
          <section role="dialog" aria-modal="true" aria-labelledby="archive-bot-title" className="w-full max-w-lg overflow-hidden rounded-md border border-edge bg-panel shadow-2xl">
            <header className="flex items-center justify-between gap-4 border-b border-edge p-5">
              <div><p className="ui-label text-gold-400">Bot lifecycle</p><h2 id="archive-bot-title" className="mt-2 t-title font-semibold text-ink">Archive {archiveTarget.name}?</h2></div>
            ...
            <div className="space-y-4 p-5">
              <p className="t-body leading-6 text-dim">The bot leaves active management and cannot open new positions. Its versions, executions, performance, and financial history remain available.</p>
              <div className="rounded-md border border-edge bg-void p-3 t-label text-dim"><span className="font-semibold text-ink">Open positions:</span> {archiveTarget.openTrades || 0}</div>
              <div className="flex justify-end gap-2">
                <button type="button" onClick={() => setArchiveTarget(null)} disabled={busy === archiveTarget.id} className="min-h-11 rounded-md border border-edge px-4 t-body font-semibold text-ink disabled:opacity-40">Cancel</button>
                <button
                  type="button"
                  ...
                  className="inline-flex min-h-11 items-center gap-2 rounded-md bg-down px-4 t-body font-semibold text-white disabled:opacity-40"
                >
