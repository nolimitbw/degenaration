...
export default function BotActivityPage() {
  ...
  if (!authenticated) {
    return <AppShell><PageHeader eyebrow="Bots / Activity" title="Signals and executions" description="Inspect the journal attached to one of your bots." /><ProductTabs items={TABS} active="/bots/manage" /><div className="mt-6"><EmptyState icon={Activity} title="Connect to inspect this bot" description="Bot journals are private to the owning account." action={<button type="button" onClick={login} className="min-h-11 rounded-md bg-gold-400 px-4 text-sm font-semibold text-[#17110c]">Connect account</button>} /></div></AppShell>;
  }

  return (
    <AppShell>
      <PageHeader
        eyebrow="Bots / Activity"
        ...
        actions={<Link href="/bots/manage" className="inline-flex min-h-11 items-center rounded-md border border-edge px-4 text-sm font-semibold text-ink">Back to My Bots</Link>}
      />
