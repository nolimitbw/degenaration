      ...
      <div className="terminal-hero flex flex-wrap items-center justify-between gap-4 rounded-lg border border-edge bg-panel p-4">
        ...
        <div className="flex w-full gap-2 sm:w-auto">
          <input value={mint} onChange={(e) => setMint(e.target.value)} disabled={tradeLocked} placeholder="Token mint address"
            className="min-h-11 min-w-0 flex-1 rounded-md border border-edge bg-void px-3 py-2 font-mono text-xs outline-none focus:border-gold-400 disabled:cursor-not-allowed disabled:opacity-50 sm:w-80" />
          <button onClick={() => void load()} disabled={loading || tradeLocked || !mintOk} className="min-h-11 rounded-md bg-gold-400 px-4 py-2 text-sm font-bold text-white shadow-gold transition hover:brightness-110 disabled:opacity-50">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-3">
          ...
          <div className="rounded-lg border border-edge bg-panel p-5">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right font-mono text-xs">
                <p className={chg >= 0 ? "text-up" : "text-danger"}>
            ...
            <div className="mt-3 flex gap-1">
              {(["chart", "holders", "info"] as const).map((t) => (
                <button key={t} onClick={() => setChartTab(t)}
                  className={`rounded px-3 py-1.5 font-mono text-[11px] font-bold transition ${chartTab === t ? "bg-gold-400 text-white" : "border border-edge text-dim hover:text-ink"}`}>{t.toUpperCase()}</button>
              ))}
            </div>
            <div className="mt-3 grid grid-cols-2 gap-2 border-t border-edge pt-3 font-mono text-[11px] sm:grid-cols-4">
              ...
              <div><p className="text-dim">Buys/Sells</p><p><span className="text-gold-400">{livePrice?.buys24h ?? "—"}</span><span className="text-dim">/</span><span className="text-danger">{livePrice?.sells24h ?? "—"}</span></p></div>
                ...
                <div className="flex gap-1">
                  {(["minute", "hour", "day"] as const).map((value) => (
                    <button
                      ...
                      className={`min-h-7 px-2 font-mono text-[9px] uppercase transition ${tf === value ? "bg-gold-400 text-[#17110c]" : "border border-edge text-dim hover:text-ink"}`}
                    >
            ...
            {chartTab === "holders" && (
              <div className="mt-3 overflow-hidden rounded-md border border-edge">
                <table className="w-full text-left text-xs"><thead className="bg-panel font-mono uppercase text-dim"><tr><th className="px-3 py-2">#</th><th className="px-3 py-2">Holder</th><th className="px-3 py-2">Amount</th><th className="px-3 py-2">%</th></tr></thead>
                  <tbody>{holders.length ? holders.map((h) => (<tr key={h.address} className="border-t border-edge font-mono"><td className="px-3 py-2">{h.rank}</td><td className="px-3 py-2 text-info">{h.address.slice(0, 4)}…{h.address.slice(-4)}</td><td className="px-3 py-2">{Math.round(h.amount).toLocaleString()}</td><td className="px-3 py-2 text-gold-400">{h.pct != null ? h.pct.toFixed(2) + "%" : "—"}</td></tr>)) : <tr><td colSpan={4} className="px-3 py-6 text-center text-dim">{tokenLoaded ? "Holder data is unavailable for this token." : "Load a token to see holders."}</td></tr>}</tbody></table>
              </div>
            )}
            {chartTab === "info" && (
        ...
        <div className="rounded-lg border border-edge bg-panel p-5">
            ...
            {(["buy", "sell", "limit"] as const).map((m) => (
              <button key={m} onClick={() => setMode(m)} disabled={tradeLocked}
                className={`rounded py-2 font-bold uppercase transition ${
                  mode === m
                    ? m === "sell" ? "bg-danger text-white" : "bg-gold-400 text-white"
                ...
                <div className="mt-1 grid grid-cols-4 gap-1">
                  {SELL_PCTS.map((p) => (
                    <button key={p} onClick={() => setSellPct(p)} disabled={tradeLocked}
                      className={`rounded border py-2 font-mono text-xs font-bold transition disabled:cursor-not-allowed disabled:opacity-50 ${sellPct === p ? "border-danger bg-danger/15 text-danger" : "border-edge text-dim hover:text-ink"}`}>{p}%</button>
                  ))}
                </div>
              ...
              <div className="mt-2 grid grid-cols-4 gap-1">
                {validPresetAmounts.map((a) => (
                  <button key={a} onClick={() => setAmount(a)} disabled={tradeLocked}
                    className="rounded border border-edge py-1.5 font-mono text-xs text-dim transition hover:border-gold-400 hover:text-gold-400 disabled:cursor-not-allowed disabled:opacity-50">{a}</button>
                ))}
              </div>
            </>
          )}
            ...
            <div className="mt-4 space-y-2 text-sm">
              {[
                ...
                <label key={o.l} className="flex items-center gap-2 text-dim">
                  <input type="checkbox" checked={o.v} onChange={(e) => o.set(e.target.checked)} disabled={executing || previewLoading} className="accent-gold-400 disabled:opacity-50" />
                  {o.l}
                </label>
              ))}
            </div>
          ...
          {mode === "buy" && (livePrice?.priceUsd || quote?.outAmount) && (
            <p className="mt-4 rounded-md border border-edge bg-void px-3 py-2 font-mono text-[11px] text-gold-400">
          ...
          {mode === "limit" ? (
            <button onClick={createLimit} disabled={!limitDraftOk || (authenticated && !canCreateLimit)}
              className="mt-4 w-full rounded-md bg-gold-400 py-3 font-bold text-white shadow-gold transition hover:brightness-110 disabled:opacity-50">
            ...
            <button onClick={runSellPreview} disabled={previewLoading || (authenticated && (!mintOk || !slippageOk || !liveBal || liveBal.uiAmount <= 0))}
              className="mt-4 w-full rounded-md bg-danger py-3 font-bold text-white shadow-danger transition hover:brightness-110 disabled:opacity-50">
          ...
          <div className="w-full max-w-md rounded-lg border border-edge bg-panel p-6" onClick={(e) => e.stopPropagation()}>
            ...
            {previewLoading ? (
              ...
              <div className="mt-4 space-y-3">
                <p className="font-mono text-sm text-danger">{preview.error}</p>
                <button onClick={previewAgain} disabled={previewLoading || executing}
                  className="w-full rounded-md border border-gold-400/60 py-2.5 text-sm font-bold text-gold-400 transition hover:bg-gold-400/10 disabled:opacity-50">
                  Preview again
                </button>
              </div>
            ) : preview?.side === "sell" ? (
              <div className="mt-4 space-y-2 font-mono text-sm">
                ...
                <div className="flex justify-between"><span className="text-dim">You receive (est.)</span><span className="text-gold-400">{preview.solOut != null ? `${preview.solOut.toFixed(4)} SOL` : "—"}</span></div>
                <div className="flex justify-between"><span className="text-dim">Price impact</span><span className={preview.priceImpactPct > 10 ? "text-danger" : ""}>{preview.priceImpactPct != null ? `${Math.abs(preview.priceImpactPct).toFixed(2)}%` : "—"}</span></div>
                ...
                {executionStatus && <p className={`rounded-md border px-3 py-2 text-[11px] ${executing ? "border-edge bg-void text-dim" : "border-danger/40 bg-danger/5 text-danger"}`}>{executionStatus}</p>}
                <button onClick={confirmTrade} disabled={executing}
                  className="mt-2 w-full rounded-md bg-danger py-3 font-bold text-white shadow-danger transition hover:brightness-110 disabled:opacity-50">
              ...
              <div className="mt-4 space-y-2 font-mono text-sm">
                ...
                <div className="flex justify-between"><span className="text-dim">Price impact</span><span className={preview.priceImpactPct > 10 ? "text-danger" : ""}>{preview.priceImpactPct.toFixed(2)}%</span></div>
                ...
                <div className="flex justify-between"><span className="text-dim">Route</span><span className="text-dim">{(preview.route || []).join(" → ") || "—"}</span></div>
                {preview.warn && <p className="rounded-md border border-danger/40 bg-danger/5 px-3 py-2 text-[11px] text-danger">{preview.warn}</p>}
                {executionStatus && <p className={`rounded-md border px-3 py-2 text-[11px] ${executing ? "border-edge bg-void text-dim" : "border-danger/40 bg-danger/5 text-danger"}`}>{executionStatus}</p>}
                <button onClick={confirmTrade} disabled={executing}
                  className="mt-2 w-full rounded-md bg-gold-400 py-3 font-bold text-white shadow-gold transition hover:brightness-110 disabled:opacity-50">
