  ...
  return (
    <div className="space-y-6">
      <div className={`rounded-md border p-5 ${report.ok ? "border-up/35 bg-up/5" : "border-danger/35 bg-danger/5"}`}>
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 t-body leading-6 text-dim">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section>
          <h2 className="t-body font-bold">Blocking reasons</h2>
          <div className="mt-3 rounded-md border border-edge bg-panel">
            {report.reasons.length ? report.reasons.map((reason) => (
              <div key={reason} className="flex items-start gap-2 border-b border-edge px-4 py-3 t-body last:border-b-0">
                <AlertTriangle aria-hidden="true" size={16} className="mt-0.5 shrink-0 text-danger" /> {reason}
              </div>
            )) : <p className="px-4 py-5 t-body text-dim">No blocking reasons were returned.</p>}
          </div>
        </section>
        <section>
          <h2 className="t-body font-bold">Provider signals</h2>
          <div className="mt-3 rounded-md border border-edge bg-panel">
            {report.providerRisks.length ? report.providerRisks.slice(0, 8).map((risk, index) => (
              <div key={`${risk.name}-${index}`} className="border-b border-edge px-4 py-3 last:border-b-0">
                <div className="flex items-center justify-between gap-3"><p className="t-body font-semibold">{risk.name}</p><span className="ui-label">{risk.level}</span></div>
                {risk.description && <p className="mt-1 t-label leading-5 text-dim">{risk.description}</p>}
              </div>
            )) : <p className="px-4 py-5 t-body text-dim">No additional provider warnings were returned.</p>}
          </div>
        </section>
      </div>

      <div className="flex flex-wrap gap-2">
        <Link href={`/terminal?mint=${mint}`} className="inline-flex min-h-11 items-center rounded-md bg-gold-400 px-4 t-body font-semibold text-[#031018]">Open terminal</Link>
        {report.pairUrl && <a href={report.pairUrl} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center gap-2 rounded-md border border-edge px-4 t-body font-semibold hover:border-gold-400/60">Open market <ExternalLink aria-hidden="true" size={15} /></a>}
      </div>
      <p className="t-label leading-5 text-dim">A passing report is not a guarantee of safety or future performance. Checks are live and can change between requests.</p>
    </div>
  );
}

function Check({ label, value, pass }: { label: string; value: string; pass: boolean }) {
  return <div className="bg-panel p-4 sm:border-r sm:border-edge sm:last:border-r-0"><p className="ui-label">{label}</p><p className={`mt-2 t-title font-bold ${pass ? "text-up" : "text-danger"}`}>{value}</p></div>;
}

function State({ title, detail, tone }: { title: string; detail: string; tone: "warn" | "loading" }) {
  return <div className="flex min-h-56 items-center justify-center rounded-md border border-edge bg-panel/40 p-6 text-center"><div>{tone === "loading" ? <LoaderCircle aria-hidden="true" className="mx-auto animate-spin text-gold-400" /> : <AlertTriangle aria-hidden="true" className="mx-auto text-danger" />}<p className="mt-3 font-bold">{title}</p><p className="mt-1 t-body text-dim">{detail}</p></div></div>;
}
