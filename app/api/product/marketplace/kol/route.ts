...
const SORTS = new Set(["performance", "drawdown", "followers", "newest", "fee"]);

export async function GET(req: NextRequest) {
  const limited = await distributedRateLimit(req, { limit: 90, windowSeconds: 60 });
  if (limited) return limited;
  const period = normalizePeriod(req.nextUrl.searchParams.get("period"), ["1d", "7d", "30d", "3m"], "30d");
  ...
  const result = await callAppBridge("app_public_list_kol_marketplace", {
    p_period: period,
    p_sort: sort,
    p_limit: 100
  });
