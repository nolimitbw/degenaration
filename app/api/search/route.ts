...
export async function GET(req: NextRequest) {
  ...
  try {
    const d = await fetch(`https://api.dexscreener.com/latest/dex/search?q=${encodeURIComponent(q)}`, { cache: "no-store" }).then(r => r.json());
    const seen = new Set<string>(); const results: any[] = [];
    for (const p of d?.pairs ?? []) {
      if (p.chainId !== "solana") continue;
      const a = p.baseToken?.address; if (!a || seen.has(a)) continue; seen.add(a);
      results.push({ address: a, symbol: p.baseToken.symbol, name: p.baseToken.name, priceUsd: p.priceUsd, mc: p.marketCap ?? p.fdv ?? null, image: p.info?.imageUrl ?? null });
