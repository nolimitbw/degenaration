  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-6xl">
            ...
            <div className="flex min-w-0 items-center gap-4">
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <h1 className="truncate t-display font-bold sm:t-display">{source.name}</h1>
                ...
                <p className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 font-mono t-label text-dim">
                ...
                {source.ownerDisplayName && <p className="mt-2 t-label text-dim">Managed by {source.ownerDisplayName}</p>}
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
              {invite && (
                <a href={invite} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center gap-2 rounded-md bg-gold-400 px-4 t-body font-semibold text-[#031018] transition hover:brightness-110">
                  <MessageCircle aria-hidden="true" size={17} /> Join Discord <ExternalLink aria-hidden="true" size={14} />
                </a>
              )}
              <Link href={`/bots/discord/new?source=${source.id}`} className="inline-flex min-h-11 items-center gap-2 rounded-md border border-edge px-4 t-body font-semibold text-ink transition hover:border-gold-400/60">
        ...
        {source.bio && <p className="mt-5 max-w-3xl t-body leading-6 text-dim">{source.bio}</p>}
        ...
        <section aria-labelledby="performance-heading" className="mt-6">
          <div className="flex items-center gap-2">
            ...
            <h2 id="performance-heading" className="t-body font-bold">All-time recorded performance</h2>
          </div>
          ...
          <p className="mt-2 flex items-start gap-2 t-label leading-5 text-dim">
        ...
        <section className="mt-7 grid min-w-0 gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
          <div className="min-w-0">
            <h2 className="t-body font-bold">Latest calls</h2>
            <div className="mt-3 overflow-x-auto rounded-md border border-edge">
              {source.recentCalls.length ? (
                <table className="w-full min-w-[980px] text-left">
                  ...
                  <tbody>
                    {source.recentCalls.map((call) => (
                      <tr key={call.id} className="border-t border-edge font-mono t-label">
                ...
                <div className="grid min-h-48 place-items-center bg-panel/30 px-5 text-center">
                  <div><p className="t-body font-semibold text-ink">No calls recorded yet</p><p className="mt-1 max-w-sm t-label leading-5 text-dim">This approved source starts with a clean record. Calls appear here only after the bot observes them in an approved channel.</p></div>
                </div>
              )}
            </div>
          </div>
          ...
          <aside className="h-fit rounded-md border border-edge bg-panel p-5">
            ...
            <p className="mt-2 t-label text-ink">{source.profileSyncedAt ? new Date(source.profileSyncedAt).toLocaleString() : "Awaiting first Discord metadata sync"}</p>
            <div className="my-4 border-t border-edge" />
            <p className="ui-label">Server referral</p>
            <p className="mt-2 break-all font-mono t-body text-ink">/r/{source.referralCode}</p>
            <p className="mt-2 t-label leading-5 text-dim">A stable public link assigned to this Discord server. It opens this profile without requiring a wallet.</p>
...
function Stat({ label, value, detail, accent = false }: { label: string; value: string; detail: string; accent?: boolean }) {
  return (
    <div className="bg-panel p-4 sm:border-r sm:border-edge sm:last:border-r-0">
      ...
      <p className={`mt-2 t-display font-bold ${accent ? "text-gold-400" : "text-ink"}`}>{value}</p>
