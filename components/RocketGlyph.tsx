...
export default function RocketGlyph({ size = 34, flame = true }: { size?: number; flame?: boolean }) {
  ...
  const bodyId = `rg-body-${id}`;
  const finId = `rg-fin-${id}`;
    ...
    <svg width={size} height={size * 1.6} viewBox="0 0 40 64" xmlns="http://www.w3.org/2000/svg" style={{ filter: "drop-shadow(0 4px 8px rgba(0,0,0,0.5))" }}>
      <defs>
        ...
        <radialGradient id={glowId} cx="50%" cy="20%" r="80%">
          ...
          <stop offset="40%" stopColor="#ff7799" />
          <stop offset="100%" stopColor="rgba(255,34,85,0)" />
        </radialGradient>
      </defs>
      {flame && <ellipse cx="20" cy="56" rx="4.5" ry="8" fill={`url(#${glowId})`} />}
      <path d="M14 40 L6 52 L14 49 Z" fill={`url(#${finId})`} />
      <path d="M26 40 L34 52 L26 49 Z" fill={`url(#${finId})`} />
      <path d="M20 2 C29 12 30 26 30 38 L30 50 L10 50 L10 38 C10 26 11 12 20 2 Z" fill={`url(#${bodyId})`} />
      <path d="M17 8 C13 20 13 36 14 50 L18 50 C16 34 16 20 19 6 Z" fill="rgba(255,255,255,0.5)" opacity="0.55" />
      <circle cx="20" cy="26" r="5" fill="#0a0204" stroke="#ff7799" strokeWidth="1.6" />
      <path d="M10 44 H30" stroke="#ff3366" strokeWidth="2.4" opacity="0.85" />
    </svg>
  );
}
