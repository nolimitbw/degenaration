  ...
  if (!authenticated) {
    return (
      <button
        type="button"
        onClick={login}
        className="inline-flex min-h-11 sm:min-h-11 sm:min-h-10 items-center gap-2 rounded-md bg-gold-400 px-3 text-xs font-semibold text-[#17110c] transition hover:brightness-110"
      >
  ...
  return (
    <div ref={menuRef} className="relative">
      <button
        ...
        aria-expanded={open}
        aria-haspopup="menu"
        className="inline-flex min-h-11 sm:min-h-11 sm:min-h-10 items-center gap-2 rounded-md border border-gold-400/55 px-3 font-mono text-xs text-gold-400 transition hover:border-gold-400 hover:bg-gold-400/5"
      >
        <Wallet aria-hidden="true" size={14} />
        {short}
      </button>
      ...
      {open && (
        <div role="menu" className="absolute right-0 top-12 z-50 w-[min(88vw,320px)] overflow-hidden rounded-md border border-edge bg-panel shadow-2xl">
          <div className="border-b border-edge px-4 py-3">
            <p className="text-xs font-semibold text-ink">Wallet account</p>
            {email && <p className="mt-1 truncate text-[11px] text-dim">{email}</p>}
          </div>
          ...
          {address ? (
            ...
            <div className="border-b border-edge p-3">
              <p className="text-xs font-medium text-ink">No Solana wallet linked</p>
              <p className="mt-1 text-[10px] leading-4 text-dim">Create a secured embedded wallet or link an existing Solana wallet.</p>
              <button
                type="button"
                onClick={createSolanaWallet}
                disabled={busy === "create"}
                className="mt-3 inline-flex min-h-11 sm:min-h-11 sm:min-h-10 w-full items-center justify-center gap-2 rounded-md bg-gold-400 px-3 text-xs font-semibold text-[#17110c] disabled:opacity-50"
              >
              ...
              <button
                ...
                className="mt-2 inline-flex min-h-11 sm:min-h-11 sm:min-h-10 w-full items-center justify-center gap-2 rounded-md border border-edge px-3 text-xs font-semibold text-ink"
              >
          ...
          <div className="p-2">
            <Link
              ...
              className="flex min-h-11 sm:min-h-11 sm:min-h-10 items-center gap-3 rounded-md px-3 text-xs font-medium text-ink transition hover:bg-void"
            >
            ...
            <button
              ...
              className="mt-1 flex min-h-11 sm:min-h-11 sm:min-h-10 w-full items-center gap-3 rounded-md border-t border-edge px-3 text-left text-xs font-medium text-down transition hover:bg-down/5 disabled:opacity-50"
            >
