...
function resolveTier(regions, prefix) {
  ...
  const directLinkMints = new Set();
  const pairLinkCandidates = new Set();
  ...
  for (const region of regions) {
    for (const candidate of linksIn(region.text)) {
      ...
      if (candidate.candidateType === "dexscreener_address") pairLinkCandidates.add(candidate.address);
      else directLinkMints.add(candidate.address);
    }
  ...
  // A direct token link is stronger evidence than an auxiliary chart/pool link. Real alert
  // embeds commonly include one GMGN/Jupiter token URL plus DexScreener, Photon and Axiom
  // pool buttons. Treating every linked address as equal rejected every such call as
  // ambiguous even though the alert identified exactly one mint. We still refuse two
  // distinct DIRECT token links: that remains genuinely ambiguous and must never be guessed.
  if (directLinkMints.size === 1) {
    const [mint] = directLinkMints;
    return { mint, confidence: label(linkOrigin, "link"), candidateType: "mint" };
  }
  if (directLinkMints.size > 1) {
    return { rejected: "ambiguous-links", candidates: directLinkMints.size, addresses: [...directLinkMints] };
  }

  // With no direct token URL, one pool link remains resolvable downstream. Several different
  // pools are not assumed to represent the same token.
  if (pairLinkCandidates.size === 1) {
    const [mint] = pairLinkCandidates;
    return { mint, confidence: label(linkOrigin, "link"), candidateType: "dexscreener_address" };
  }
  ...
  if (linkMints.size > 1) {
    // The addresses themselves, not just how many. A refusal that says "2 candidates" tells
    // the owner their call vanished; one that names both tells them whether the parser was
    // right. Ordered as encountered, because the caller almost always leads with the token.
    return { rejected: "ambiguous-links", candidates: linkMints.size, addresses: [...linkMints] };
  }
  if (addresses.size === 1) {
  ...
  if (addresses.size > 1) {
    return { rejected: "ambiguous-addresses", candidates: addresses.size, addresses: [...addresses] };
  }
  return null;
}
