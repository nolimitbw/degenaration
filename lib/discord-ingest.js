...
function normalizeIngestRequest(body) {
  ...
  // A delete carries no mint by construction: the message it retracts is gone.
  if (eventType !== "delete" && !isMint(source.mint)) {
    return { ok: false, error: "invalid mint", status: 400 };
  }

  const mint = eventType === "delete" ? null : source.mint;
  return {
    ok: true,
...
function buildIngestRpcParams({ normalized, price, secret }) {
  return {
    ...
    p_symbol: price?.symbol ?? null,
    p_called_mcap: price?.calledMcap ?? null,
    p_called_price_usd: price?.calledPrice ?? null,
    p_called_liquidity_usd: price?.calledLiquidity ?? null,
