 ...
 */
/**
 * How long after a message is posted its live price still counts as its ENTRY price.
 *
 * A backfilled message gets no `called_price_usd`, and that is right: DexScreener answers with
 * the price NOW, and stamping today's price onto a call from three weeks ago would invent the
 * source's entire track record. Every measured statistic on the marketplace is a ratio against
 * that baseline.
 *
 * But "arrived via the backfill" is not the same fact as "is old", and treating them as one had
 * a consequence nobody intended. The live listener quarantines everything it sees — 116 messages
 * against 5 forward attempts, the last on 2026-08-05 — so the history scanner is currently the
 * ONLY path a call takes. Every call therefore arrived flagged historical, every baseline was
 * skipped, and the marketplace could never measure anything: 1 of 2,275 calls priced. Source
 * owners had nothing to show, which is the whole product.
 *
 * The scanner runs every two minutes. A message it picks up seconds after posting has, for this
 * purpose, the same price it had when posted. Five minutes is the tolerance: wide enough to
 * survive a missed cycle or a slow run, narrow enough that the number still means what the
 * column says. Anything older stays unpriced and unmeasured rather than guessed.
 */
const LIVE_CAPTURE_WINDOW_MS = 5 * 60 * 1000;

function isHistorical(eventVersion, nowMs = Date.now()) {
  if (!eventVersion?.startsWith("history:")) return false;
  const postedAt = Date.parse(eventVersion.slice("history:".length));
  // An unparseable stamp is treated as old. Failing towards "no baseline" loses a data point;
  // failing the other way publishes a fabricated one.
  if (!Number.isFinite(postedAt)) return true;
  return nowMs - postedAt > LIVE_CAPTURE_WINDOW_MS;
}
...
function normalizeIngestRequest(body) {
  ...
  const historicalBackfill = isHistorical(eventVersion);
