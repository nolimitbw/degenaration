 ...
 * available.
 */
function plannedCapital({ buyAmountSol, entryLamports: entryOverride, maxOpenTrades, dca, perTokenExposureSol } = {}) {
  // `entryLamports` lets a caller holding the PERSISTED integer skip the SOL round trip. The
  // builder passes SOL because that is what the user typed; the server passes lamports because
  // that is what was saved, and converting a stored integer back through a float to reach the
  // same integer is a needless opportunity to disagree with it.
  const entryLamports = entryOverride === undefined || entryOverride === null
    ? toLamports(buyAmountSol)
    : (() => { try { const value = BigInt(entryOverride); return value > 0n ? value : 0n; } catch { return 0n; } })();
