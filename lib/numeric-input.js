...
function normalizeWhileTyping(text) {
  ...
  // "05" -> "5", but "0" and "0.5" are untouched. The `|| "0"` matters: without it an
  // all-zero body like "00" stripped to "" and the field appeared to erase itself as the
  // user typed a second zero.
  if (/^0\d/.test(body)) body = body.replace(/^0+/, "") || "0";
...
function clamp(value, min, max) {
  ...
  // The `max > 0` guard that used to be here silently ignored a max of 0 or below, so
  // clamp(50, 0, 0) returned 50. No caller uses 0 to mean "unlimited" — every max in the
  // product is a real positive limit — so the guard only ever hid a bug.
