    ...
    try {
      ...
      if (token) {
        await fetchWithTimeout("/api/record-trade", {
          ...
          body: JSON.stringify({ mint: args.mint, side: "buy", solAmount: args.solAmount, priceUsd: args.priceUsd, feeSol: args.solAmount * 0.02, sig, kind: "manual", userPubkey: embeddedAddr })
        }).catch(() => {});
      }
      return { ok: true, sig };
    } catch (e: any) {
