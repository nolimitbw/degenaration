...
function sourceAggregate(calls, options) {
  ...
  return {
    ...
    minimumSampleSize,
    // A marketplace hit starts at the first meaningful milestone: +50%. Counting any peak
    // above 1.00x turns ordinary quote noise into a "win" and made real sources display
    // ~96% hit rates even while most calls were underwater. Keep 2x separate.
    winRate: multiples.length ? (multiples.filter((value) => value >= 1.5).length / multiples.length) * 100 : null,
