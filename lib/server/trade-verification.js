...
function analyzeSwapTransaction(transaction, expected) {
  ...
  const absoluteDelta = tokenDelta < 0n ? -tokenDelta : tokenDelta;
  ...
  return {
    ...
    feeSol,
    decimals,
    // Human units for the ledger; RAW units for anything that has to sell this position
    // later — a sell is quoted in raw amounts, and selling more than the wallet holds
    // fails on-chain.
    tokenAmount: Number(absoluteDelta) / (10 ** decimals),
    tokenAmountRaw: absoluteDelta.toString()
  };
}
