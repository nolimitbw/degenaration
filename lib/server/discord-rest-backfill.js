...
// The one definition of "too old to carry a call-time price". Asked rather than
// duplicated, so the label this file writes and the rule the ingest applies cannot drift.
const { isHistorical } = require("../discord-ingest");
...
/**
 * The event version says what this message IS, which until now it did not.
 *
 * Both directions used one builder and stamped everything `history:` — including the forward
 * sweep that catches a call two seconds after it is posted. A live call was therefore labelled
 * as archive, and `fan_out_on_parse` skips any signal whose event_version starts with
 * `history:`. That is the whole reason signal_deliveries sat at zero across 2,309 calls: not a
 * broken join, a guard doing exactly what it says on a pipeline where every row was mislabelled.
 *
 * The archive walk keeps `history:`; those calls must stay unpriced rather than carry a
 * baseline invented days later. A forward-sweep message is `original` unless it is old enough
 * that isHistorical would refuse to price it anyway — which covers the catch-up case after a
 * scanner outage without a second age rule living here.
 */
function buildIngestPayload({ channel, message, parsed, archiveWalk }) {
  const historyVersion = `history:${new Date(message.timestamp).toISOString()}`;
  const eventVersion = archiveWalk || isHistorical(historyVersion) ? historyVersion : "original";
  return {
...
async function scanDiscordHistoryPage({ channel, state, token, ingest, saveState, fetchImpl = fetch }) {
  ...
  for (let index = 0; index < messages.length; index += INGEST_CONCURRENCY) {
    ...
    const results = await Promise.all(batch.map(async (message) => {
      ...
      return ingest(buildIngestPayload({ channel, message, parsed, archiveWalk: !completed }));
    }));
