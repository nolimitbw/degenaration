...
function discordRetryDelay(response, body) {
  const headerSeconds = Number(response.headers?.get?.("retry-after"));
  const bodySeconds = Number(body?.retry_after);
  const seconds = Number.isFinite(headerSeconds) && headerSeconds > 0
    ? headerSeconds
    : (Number.isFinite(bodySeconds) && bodySeconds > 0 ? bodySeconds : 1);
  // Leave enough time for the bounded Render scan request to finish and report truthfully.
  return Math.min(45_000, Math.max(250, Math.ceil(seconds * 1000)));
}

async function discordJson(path, token, fetchImpl = fetch, sleepImpl = delay => new Promise(resolve => setTimeout(resolve, delay))) {
  ...
  if (response.status === 429) {
    ...
    const delay = discordRetryDelay(response, limited);
