 ...
 */
const EXECUTION_GLOBS = ["server/engine", "server/worker.js"];
...
const BOT_CONTROL_CONTRACT = [
  ...
  { path: "channelId", label: "Specific channel", section: "source", kinds: ["discord"],
    ...
    reason: "The bot follows every approved channel of this source. Restricting it to one channel needs the worker to filter calls by channel, which it does not yet do." },
  ...
  { path: "maximumCapitalLamports", label: "Maximum capital", section: "funding", kinds: ["discord", "kol"],
    status: "pending", blocker: null,
    reason: "Saved and shown in the capital summary. The daily loss limit is the cap the engine enforces today; a separate lifetime capital ceiling has no reader." },
  { path: "perTokenExposureLamports", label: "Per-token exposure limit", section: "funding", kinds: ["discord", "kol"],
    status: "pending", blocker: null,
    reason: "No reader. Exposure is currently bounded by the buy amount and the daily loss limit, not per token." },
  { path: "maxOpenTrades", label: "Maximum open trades", section: "funding", kinds: ["discord", "kol"],
    status: "pending", blocker: null,
    reason: "Saved and used to compute required capital. Nothing counts open positions against it at entry time." },
  ...
  { path: "priorityFeeStrategy", label: "Priority fee strategy", section: "execution", kinds: ["discord", "kol"],
    status: "pending", blocker: null,
    reason: "No reader. The submitter uses one priority-fee policy for every bot." },
  { path: "priorityFeeMaxLamports", label: "Priority fee cap", section: "execution", kinds: ["discord", "kol"],
    status: "pending", blocker: null,
    reason: "Validated on save and read by no execution path, so it does not cap what a submission pays." },
  { path: "autoRetryCount", label: "Auto retry", section: "execution", kinds: ["discord", "kol"],
    status: "pending", blocker: null,
    reason: "Retry bounds are fixed in the engine and are not read from the bot." },
  ...
  { path: "quoteExpirationSeconds", label: "Quote expiration", section: "execution", kinds: ["discord", "kol"],
    status: "pending", blocker: null,
    reason: "The engine applies its own quote freshness window and does not read this one." },
  { path: "cooldownSeconds", label: "Duplicate cooldown", section: "execution", kinds: ["discord", "kol"],
    status: "pending", blocker: null,
    reason: "Duplicate calls are suppressed by a fixed 60-second window at ingestion, the same for every bot." },
  { path: "simulationRequired", label: "Require simulation", section: "execution", kinds: ["discord", "kol"],
    status: "pending", blocker: null,
    reason: "Simulation is applied by policy to every submission and is not switchable per bot." },
  { path: "firstCallOnly", label: "First call only", section: "execution", kinds: ["discord"],
    status: "pending", blocker: null,
    reason: "No reader. Every eligible call from the source is treated the same." },
  ...
  { path: "takeProfit.trailing", label: "Trailing take profit", section: "takeProfit", kinds: ["discord", "kol"],
    status: "pending", blocker: null,
    reason: "Trailing is applied per level from that level's retracement, not from this switch. Set a retracement on a level to make it trail." },
  ...
  { path: "stopLoss.delaySeconds", label: "Stop delay", section: "stopLoss", kinds: ["discord", "kol"],
    status: "pending", blocker: null,
    reason: "The stop fires on the first tick past the threshold. Debounce has no reader." },
  { path: "stopLoss.freezeAfterStop", label: "Freeze token after stop", section: "stopLoss", kinds: ["discord", "kol"],
    status: "pending", blocker: null,
    reason: "No reader. A token that stopped out can be entered again on the next call." },
  { path: "stopLoss.emergencyExit", label: "Emergency exit", section: "stopLoss", kinds: ["discord", "kol"],
    status: "pending", blocker: null,
    reason: "No reader. Pausing a bot preserves exits; there is no separate emergency close." },
  ...
  { path: "trigger.stalePriceBehavior", label: "Stale price handling", section: "entry", kinds: ["kol"],
    status: "pending", blocker: "E-3", reason: "Part of the KOL entry trigger, which has no reader." },
  { path: "dca.enabled", label: "Dollar-cost averaging", section: "dca", kinds: ["kol"],
    status: "pending", blocker: "E-3",
    reason: "DCA levels are stored and counted toward required capital. No path places the additional entries." },
  { path: "dca.levels", label: "DCA levels", section: "dca", kinds: ["kol"],
    status: "pending", blocker: "E-3", reason: "Stored and counted toward required capital; no path places them." },
  { path: "dca.expirationMinutes", label: "DCA expiration", section: "dca", kinds: ["kol"],
    status: "pending", blocker: "E-3", reason: "Depends on DCA entries, which are not placed." },
  { path: "dca.maximumEntries", label: "Maximum DCA entries", section: "dca", kinds: ["kol"],
    status: "pending", blocker: "E-3", reason: "Depends on DCA entries, which are not placed." },
