...
const CHECKS = [
  ...
  {
    // Second, right after the configuration itself, because it is the one fault no other
    // check can compensate for: nothing about the wallet, the source or the balance matters
    // if the bot cannot leave its stored state.
    //
    // Archiving is terminal by design — enforce_bot_lifecycle raises "archived bot cannot be
    // restored" — but readiness did not look at the stored state at all. So the panel said
    // "Every readiness check passes" for a bot the database would always refuse, RUN failed,
    // and the failure surfaced as "temporarily unavailable. Try again shortly."
    //
    // Undefined means not-an-edit (a new bot), which is fine. Only a known-archived bot fails.
    id: "lifecycle",
    blocking: "user",
    check: (f) => f.storedStatus !== "archived"
      || "This bot is archived, and archiving is permanent. Create a new bot from the same source to trade again."
  },
  {
    id: "wallet",
    blocking: "user",
  ...
  },
  {
    id: "delegation",
    blocking: "user",
    // Delegation is checked again by Privy when a trade is signed. Its identity-token flag can
    // lag behind a successful signer grant, so it must not stop a bot entering the active,
  ...
  },
  {
    id: "capital",
    blocking: "user",
    // Funding controls execution, not activation. An active unfunded bot safely waits because
    // the claim/simulation/submission path still refuses an order the wallet cannot cover.
    check: () => true
  },
  {
    id: "emergencyStop",
