...
function spendableLamports(input) {
  ...
  const locked = toLamports((input && input.lockedLamports) || 0);
  const pending = toLamports((input && input.pendingWithdrawalLamports) || 0);
...
/** Stable key so a retried submission can never create a second transfer (§12.5). */
