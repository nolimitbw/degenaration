-- `freezeAfterStop` could not be implemented until settlement recorded why a position closed.
--
-- THE SWITCH
--
-- The builder has offered "Freeze token after stop" since it shipped. Its meaning is exact: do
-- not re-enter a token that just stopped you out, for the length of the cooldown. It has been
-- persisted, validated, versioned and reloaded the whole time, and enforced by nothing —
-- because the fact it depends on did not exist. `worker_settle_position_exit` cleared
-- `pending_exit_kind` in the same statement that closed the position, so "closed" and "stopped
-- out" were the same row and nothing could tell them apart.
--
-- degenaration-exit-reason.sql records it. This reads it.
--
-- WHY IT IS A SEPARATE REFUSAL FROM THE COOLDOWN
--
-- They are different settings with different fixes. "You traded this token recently" sends the
-- owner to the cooldown; "this token stopped you out and you asked not to re-enter yet" sends
-- them to the stop-loss section. Collapsing them into one message would send half the readers
-- to the wrong control.
--
-- SCOPE. The freeze is bounded by the SAME cooldown window rather than a new setting, because
-- the builder offers no separate freeze duration and inventing one would be a control the user
-- never chose. With the cooldown switched off there is no window, so the freeze does not apply.
--
-- Forward safety: `worker_claim_call_execution` replaced at its EXISTING arity (uuid, uuid).
-- Supersedes migration 13 in full and MUST FOLLOW IT. No column added, no constraint changed,
-- no grant altered, no row rewritten.
-- Rollback: supabase/rollback/16-freeze-after-stop.sql.
