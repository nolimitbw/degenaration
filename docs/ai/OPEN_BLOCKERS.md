...
| E-3 | Production worker | **NARROWED 2026-08-06, and the long-standing wording was wrong.** The Railway `degenaration-worker` service DOES already hold Privy delegated-signing credentials — app id, app secret and authorisation key are all set. The signer credential was never the blocker. What is actually missing: the service has **no source connected** (its last deployment is 2026-07-08 and predates the current worker), `DELEGATED_SIGNING=off`, `WORKER_NET=devnet`, and no `MAINNET_RPC`. `server/worker.js` gates the ENTIRE trading stack behind `DELEGATED_SIGNING === "on"`, so watch-only genuinely cannot claim, sign or trade. Turning it on is the one irreversible funds-moving action in the product and was deliberately not taken. | Connect the worker source and approve enabling delegated signing |
