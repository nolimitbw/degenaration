...
| E-2 | Live Discord ingestion proof | **Narrowed 2026-08-05.** The bridge is no longer "has never forwarded anything": `degencalls` now reports `ingestion: {attempts: 2, accepted: 1, failed: 1, quarantined: 55}`, and the accepted one is **correlated to the journal** — its `lastSuccessAt` of `2026-08-04T19:41:31.667Z` matches `raw_signals.received_at` of `19:41:31.431`, a delete event journaled by parser `discord-v3`. So the delivery path and the RPC both work end to end on live data. What remains unknown is the **quarantine rule**, which lives inside the `degencalls` program — not in this repository and not visible under the accessible GitHub account, so it cannot be read from here. One message in an approved channel distinguishes the two possibilities: `attempts` increments → the chain proceeds and the journal fills; `quarantined` increments and `attempts` does not → the bridge refuses well-formed calls. The Discord MCP in this environment has **no token configured**, so it cannot post that message. | BLOCKED — one owner action |
