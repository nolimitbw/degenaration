...
| **HEAD** | `d0a5bdc` |
| **Production (Vercel)** | `eef0544` — `/api/build` confirms |
| **Suite** | `npm run check`: **357 passed, 0 failed**, exit 0 |
| **Automation** | `automationLive: false` — first failing check `workerLease` |
| **Platform fee** | **0 bps collected.** `feeLabel: "None"`. The ATA does not exist on chain |
<!-- handoff:state:end -->
