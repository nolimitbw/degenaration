...
## What is blocking the product — rewritten 2026-08-12, the earlier list is stale

**One check fails, not thirteen.** Readiness is **17 of 18** on production. Two of the three
items below are closed; the third now blocks more than revenue.

1. ~~The worker has no host.~~ **Closed.** A `degenaration-worker` is on Render, up ~20h,
   `mode: watch-only`. It cannot sign, so the Vercel in-app engine remains the executor. See
   `docs/ai/RUNNING_ARCHITECTURE.md` for which does what and why both are safe.
2. ~~Delegated signing is off.~~ **Closed on Vercel**, still off on Render — deliberately.
   Turning it on there gives two signing paths; retire the `degenaration-execution-engine`
   schedule in the same change if you do.
3. **The fee token account does not exist.** Now the ONLY failing check, and it is not merely a
   revenue problem: `app/api/product/bots/route.ts:40` gates activation on this same readiness, so
   **every attempt to turn a bot on returns 503.** The product cannot be used end to end until
   one ~0.002 SOL transaction exists.

   It does **not** need the fee wallet's private key — an ATA is deterministic and anyone may
   create one for any owner. It does need a funded wallet to pay rent and sign, which is why no
   agent can do it. `docs/ai/OWNER_RUNBOOK.md` has the command.

   **Do not "unblock" this by dropping `fee` from the readiness checks.** Whether to trade while
   collecting nothing is the owner's call, not an agent's, and `CLAUDE.md` is explicit that
   autonomy does not license weakening a financial invariant.
