...
Last updated: 2026-08-22 by Codex.

## Auto-trading availability follow-up — 2026-08-22

The first repair restored the executor but `/api/platform/config` still published `live:false`
because the suspended Render scanner health page remained a blocking execution check. Scanner
availability is now advisory: it determines whether new signals arrive, not whether the Vercel
engine can safely settle, exit, reconcile, or consume already-durable intents. The Vercel tick
also truthfully reports `copyTradingEnabled:true` because it runs `startCallWatcher`.

The Wallet grant control now uses Privy's headless delegated action behind DegenAration's own
explicit consent button. This avoids waiting on a second provider modal and replaces the false
"not yet available" copy with the actionable access state. Users retain the required ability
to decline or revoke wallet authority; auto-trading cannot lawfully or technically be enabled
for a wallet without its owner's authorization.

## Copy-trading execution repair — 2026-08-22

Production diagnosis found that Discord ingestion was healthy, but execution was blocked by
two independent faults: activation treated an owned Privy wallet as delegated even when the
app had no signing authority, and readiness depended on a suspended Render worker despite the
scheduled Vercel execution engine continuing to run.

The repair preserves Privy's signed `delegated` wallet claim through the ownership boundary,
rejects every automated activation for an undelegated wallet, and reports the scheduled Vercel
engine's database heartbeat and full execution capabilities. The scheduled engine now also
validates its signing configuration before claiming calls. `AutoTrade` remains mounted in
`AppShell`, so signed-in owners receive Privy's grant prompt and can explicitly authorize or
revoke unattended execution.

Verification before deployment: TypeScript passed; 406 server tests passed; Discord ingestion,
replay, approved-channel, pipeline E2E, command uniqueness, intent fanout/reconciliation, fee
ledger, responsive UI, simulation, and mainnet no-broadcast simulation gates passed. No mainnet
transaction was signed or broadcast during verification. Browser evidence JPG changes and the
untracked Claude worktree/SQL files predated this repair and were deliberately excluded.
...
| **Last non-Claude commit** | `0061157` — **0** commits landed since |
| **Suite** | `npm run check`: **408 tests passed, 0 failed** |
| **Migrations applied** | see DATABASE_AUDIT.md |
| **Requirements** | 22 PASS · 2 PARTIAL · 0 BLOCKED |
| **Unpushed commits** | 0 |
| **Uncommitted files** | 51 |
...
## 2026-08-22 — Discord copy-trading execution repair

- Production ingestion was healthy, but recent DegenAration alerts were durably rejected as
  `ambiguous_mint`: their embeds contain one direct token link plus auxiliary pool/chart links.
  The parser now gives a unique direct token link precedence while retaining ambiguity rejection
  for multiple distinct direct token links. A regression test covers the real alert shape.
- Starting a new bot and resuming a paused bot now request Privy delegated wallet authority in the
  same user action before the readiness/execution transition. The refreshed identity token is used
  immediately, removing the stale-token and separate Wallet-page failure mode.
- Verification: typecheck, 408 server tests, Discord ingestion/replay/channel/pipeline suites,
  command uniqueness, fee ledger, visible copy, responsive evidence, simulation gate, mainnet
  quote/build simulation, and the complete `npm run check` suite passed. Automated verification
  did not sign or broadcast a mainnet transaction.
