...
| 3 | Self-service user principal withdrawal | §12 | PARTIAL | `47dec45`; endpoint + rules module + real modal; 11 unit tests. RPC applied to the live database and its locked-capital query **verified against live Postgres**: in-flight buy intents counted, sells and terminal states excluded, bad admin secret rejected. **No real signature produced — needs a funded wallet** |
...
| 5 | Discord/KOL signal journal end-to-end | §9 | PARTIAL — **root cause found** | `d792d7f`; outcome + aggregation layer implemented (`lib/call-outcomes.js`) with 12 §22.3 fixture tests: buckets, drawdown bps, distribution, minimum-sample gate, dedupe, edits. Schema + parser contract gated by `npm run verify:performance-journal`. **Root cause: `render.yaml` had NO service for the Discord bot, so the gateway listener never ran. Service added + 13 undeclared env vars declared. Needs the owner to set DISCORD_BOT_TOKEN / INGEST_URL / BOT_SHARED_SECRET and deploy — see `docs/launch/DEPLOYMENT.md`** |
