...
| 5 | Discord/KOL signal journal end-to-end | §9 | NOT STARTED | Schema exists (`raw_signals`, `parsed_signals`, `signal_deliveries`); pipeline not traced |
...
| 15 | Verification scripts wired into checks | §4.4 | PARTIAL | 3 of 4 written and wired: `verify-fee-ledger`, `check-discord-commands`, `check-visible-copy`; `verify-performance-journal` pending |
...
| 18 | Launch documentation set | §4.3 | PARTIAL | `FINAL_LAUNCH_SPEC`, `RELEASE_CHECKLIST`, `FEE_AND_REWARD_MODEL`, `DISCORD_COMMANDS` written; `UI_SYSTEM`, `UI_COPY`, `WITHDRAWAL_FLOW`, `PERFORMANCE_JOURNAL`, `RELEASE_EVIDENCE` pending |
| 19 | Visual regression evidence | §22.6 | NOT STARTED | Home and Affiliate observed live; no captured screenshot set at 4 viewports |
